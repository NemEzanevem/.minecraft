...
void main() {
    ...
    if (Type == 0) { // Shield
        ...
        float reddition = (1-Power)*2;

        float co = abs(1 - (color * 2));
        float r = 0.4 * color + reddition;
        float g = 1 - reddition/4;
        float b = 4;
        color *= Startup;
        co *= Startup;
        c = vec4(co * r, pow(co, 2.0) * g, max(0,(pow(co ,3.0) * b) - reddition*2), max((color - 0.25) * 1.5, 0.2 * tcoord.y * Startup));//0.05
